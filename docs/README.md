...
These docs describe the system as implemented so far, plus modules that are
designed but not yet written — those are marked **not yet implemented** at
the point they're introduced. For current build status and the active task
list, see [STATUS.md](../STATUS.md), not this folder.
