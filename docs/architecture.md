...
| `shell/wm.rs` | The pane's `no_focus` rule sent to sway over its IPC socket before each pane, one request per connection under a deadline | IPC socket |
| `shell/process.rs` | Short-lived helpers such as `systemctl --user`, bounded in time and output | subprocesses |
| `shell/unix_socket.rs` | A Unix-socket connect under a deadline, which a full accept queue cannot hold past it, for `shell/wm.rs`, `shell/nvim/rpc.rs` and the CLI client in `shell/control.rs`; the peer's credentials (`SO_PEERCRED`) | Unix socket |
