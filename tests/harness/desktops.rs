...
/// What moves the focus in a test's Openbox, beside the click that always
/// does.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OpenboxFocus {
    /// `followMouse no`, Openbox's default.
    Click,
    /// `followMouse yes` with `focusDelay 0`: the pointer entering a window
    /// focuses it at once. `underMouse` is left at its default, `no`.
    FollowsMouse,
    /// As [`Self::FollowsMouse`], with `underMouse yes`: Openbox also focuses
    /// whatever window comes to be under a pointer that did not move, such as
    /// one mapped there.
    UnderMouse,
}
...
/// new windows), by default no focus-follows-mouse, and two desktops so one
/// can be empty.
pub struct Openbox {
...
impl Openbox {
    pub fn start() -> Self {
        Self::start_with(OpenboxFocus::Click)
    }

    pub fn start_with(focus: OpenboxFocus) -> Self {
        let server = XServer::start();
        let private = Private::new();
        let config = private.path("rc.xml");
        let (follow, under) = match focus {
            OpenboxFocus::Click => ("no", "no"),
            OpenboxFocus::FollowsMouse => ("yes", "no"),
            OpenboxFocus::UnderMouse => ("yes", "yes"),
        };
        std::fs::write(
            &config,
            format!(
...
<openbox_config xmlns="http://openbox.org/3.4/rc">
  <focus>
    ...
    <followMouse>{follow}</followMouse>
    <focusDelay>0</focusDelay>
    <underMouse>{under}</underMouse>
  </focus>
...
impl I3 {
    ...
    /// i3 with `focus` deciding what moves the focus, on a server whose X
    /// resources are `resources` before i3 starts: i3 reads `Xft.dpi` once,
    /// and it sizes the title bar.
    pub fn start_with(focus: super::I3Focus, resources: &str) -> Self {
        let server = XServer::start();
        server.set_resources(resources);
        let i3 = super::I3::start_with(&server, focus);
        Self { i3, server }
    }

    /// i3's own IPC socket, for a command such as `[id=…] focus`.
    pub fn command(&self, command: &str) {
