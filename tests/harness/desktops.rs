...
//!   session bus of its own. (KWin 6 no longer ships `kwin_x11` in the `kwin`
//!   package, so there is no X11 KWin to run.)
//!
...
impl KwinWayland {
    ...
    pub fn start(level: FocusStealingPrevention) -> Self {
        let private = Private::new();
        let kwinrc = private.path("config").join("kwinrc");
        std::fs::write(
            &kwinrc,
            format!(
                "# generated by the spokenpad pane tests; never the user's config\n\
        ...
        let bus_socket = private.runtime().join("bus");
        let bus = Killed(
            private
                .command("dbus-daemon")
                .arg("--session")
                .arg("--nofork")
                .arg(format!("--address=unix:path={}", bus_socket.display()))
                .spawn()
                .expect("start a private session bus"),
        );
        wait_for(START_TIMEOUT, "the private session bus", || {
            bus_socket.exists().then_some(())
        });
        let display_file = private.path("display");
        let compositor = Killed(
            private
        ...
        Self {
            server,
            _bus: bus,
            bus_address: format!("unix:path={}", bus_socket.display()),
            level,
            scripts: 0,
            private,
        }
...
pub fn find_by_instance(server: &XServer, instance: &str) -> Option<Window> {
    if instance_of(server, server.root).as_deref() == Some(instance) {
        return Some(server.root);
    }
    let children = server
        .connection
        ...
        .reply()
        .ok()?;
    children.children.into_iter().find_map(|child| {
        if instance_of(server, child).as_deref() == Some(instance) {
            return Some(child);
        }
        // One level of frame is all any window manager here adds.
        server
            .connection
            .query_tree(child)
            .ok()?
            ...
            .children
            .into_iter()
            .find(|grandchild| instance_of(server, *grandchild).as_deref() == Some(instance))
    })
}
