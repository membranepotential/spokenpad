...
`start`, `stop`, `toggle` or `cancel` finding no daemon listening. On the
socket `spokenpad.socket` listens on, `start` and `toggle` first try to start
that unit once, and say why they failed in a desktop notification as well,
since a key binding's output goes nowhere. `stop` and `cancel` start nothing
and notify nobody: with no daemon there is nothing to end. The daemon itself
does not
